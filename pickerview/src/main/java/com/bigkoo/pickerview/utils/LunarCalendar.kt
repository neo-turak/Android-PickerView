...
object LunarCalendar {
    ...
    fun lunarToSolar(
        ...
        var year1 = year
        require(!(year1 < MIN_YEAR || year1 > MAX_YEAR || month < 1 || month > 12 || monthDay < 1 || monthDay > 30)) {
            "Illegal lunar date, must be like that:\n\t" +
                    "year : 1900~2099\n\t" +
                    "month : 1~12\n\t" +
                    "day : 1~30"
        }
        var dayOffset: Int = (LUNAR_INFO[year1 - MIN_YEAR] and 0x001F) - 1
        if (LUNAR_INFO[year1 - MIN_YEAR] and 0x0060 shr 5 == 2) dayOffset += 31
        var i = 1
        while (i < month) {
            dayOffset += if (LUNAR_INFO[year1 - MIN_YEAR] and (0x80000 shr i - 1) == 0) 29 else 30
            i++
        }
        dayOffset += monthDay
        val leapMonth: Int = LUNAR_INFO[year1 - MIN_YEAR] and 0xf00000 shr 20

        // 这一年有闰月
        if (leapMonth != 0) {
            if (month > leapMonth || month == leapMonth && isLeapMonth) {
                dayOffset += if (LUNAR_INFO[year1 - MIN_YEAR] and (0x80000 shr month - 1) == 0) 29 else 30
            }
        }
        if (dayOffset > 366 || year1 % 4 != 0 && dayOffset > 365) {
            year1 += 1
            dayOffset -= if (year1 % 4 == 1) 366 else 365
        }
        ...
        while (i < 13) {
            ...
            if (year1 % 4 == 0 && i == 2 && iPos + 1 == dayOffset) {
                solarInfo[1] = i
    ...
    fun solarToLunar(year: Int, month: Int, monthDay: Int): IntArray {
        ...
        val solar11: Int = solar_1_1[index]
